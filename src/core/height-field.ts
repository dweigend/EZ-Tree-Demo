/**
 * Deterministic continuous terrain height, slope, normal, and base-noise sampling.
 * Domain-warped multi-scale Three.js noise creates landforms without owning ecology or meshes.
 */
...
export class HeightField {
  ...
  public getNormal(x: number, z: number, target: Vector3, step = SURFACE_SAMPLE_DISTANCE): Vector3 {
    return this.getGradient(x, z, target, step).normalize();
  }

  private getGradient(x: number, z: number, target: Vector3, step = SURFACE_SAMPLE_DISTANCE): Vector3 {
    const dx = this.getHeight(x + step, z) - this.getHeight(x - step, z);
    const dz = this.getHeight(x, z + step) - this.getHeight(x, z - step);
    return target.set(-dx, step * 2, -dz);
  }

  public getNoise01(domainX: number, domainZ: number, octaves = 3): number {
    return this.fbm(domainX, domainZ, octaves) * 0.5 + 0.5;
  }
